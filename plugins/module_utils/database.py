...
PATTERN_3 = re.compile(
    r";.*(SELECT|UPDATE|INSERT|DELETE|DROP|TRUNCATE|ALTER)", re.IGNORECASE
)
...
def pg_quote_identifier(identifier, id_type):
    ...
    if len(identifier_fragments) > _PG_IDENTIFIER_TO_DOT_LEVEL[id_type]:
        raise SQLParseError(
            "PostgreSQL does not support %s with more than %i dots"
            % (id_type, _PG_IDENTIFIER_TO_DOT_LEVEL[id_type])
        )
...
def mysql_quote_identifier(identifier, id_type):
    ...
    if (len(identifier_fragments) - 1) > _MYSQL_IDENTIFIER_TO_DOT_LEVEL[id_type]:
        raise SQLParseError(
            "MySQL does not support %s with more than %i dots"
            % (id_type, _MYSQL_IDENTIFIER_TO_DOT_LEVEL[id_type])
        )
