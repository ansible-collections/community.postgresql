...
def prohibited_output_profile(string):
    ...
    for c in string:
        ...
        if in_table_c12(c):
            raise ValueError(
                "%s: prohibited non-ASCII space characters "
        ...
        if in_table_c6(c):
            raise ValueError(
                "%s: prohibited inappropriate for plain text " "characters (C.6)." % RFC
            )
        if in_table_c7(c):
            raise ValueError(
                "%s: prohibited inappropriate for canonical "
                "representation characters (C.7)." % RFC
            )
        if in_table_c8(c):
            raise ValueError(
                "%s: prohibited change display properties / "
