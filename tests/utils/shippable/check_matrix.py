...
def main():  # type: () -> None
    ...
    if not defined_matrix:
        fail(
            'No matrix entries found in the "shippable.yml" file.',
    ...
    for attempts_remaining in range(4, -1, -1):
        try:
            jobs = json.loads(
                open_url("https://api.shippable.com/jobs?runIds=%s" % run_id).read()
            )
            ...
            sys.stderr.write(
                "Unable to retrieve Shippable run %s matrix: %s\n" % (run_id, ex)
            )
    ...
    if len(jobs) != len(defined_matrix):
        ...
        fail(
            "Shippable run %s has %d jobs instead of the expected %d jobs."
            % (run_id, len(jobs), len(defined_matrix)),
    ...
    actual_matrix = dict(
        (
            job.get("jobNumber"),
            dict(tuple(line.split("=", 1)) for line in job.get("env", [])).get("T", ""),
        )
        for job in jobs
    )
    errors = [
    ...
    if len(errors):
        error_summary = "\n".join(
            'Job %s expected "%s" but found "%s" instead.'
            % (job_number, expected, actual)
