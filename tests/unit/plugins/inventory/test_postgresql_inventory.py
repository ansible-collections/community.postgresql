...
class TestPostgreSQLInventoryPlugin:
    ...
    def test_verify_file_valid(self, inventory_plugin):
        ...
        assert inventory_plugin.verify_file("tests/unit/plugins/inventory/inventory.pg_inv.yml") is True
        assert inventory_plugin.verify_file("tests/unit/plugins/inventory/inventory.pg_inv.yaml") is True

    def test_verify_file_invalid(self, inventory_plugin):
        """Test that verify_file rejects invalid file extensions."""
        assert inventory_plugin.verify_file("tests/unit/plugins/inventory/inventory.ini") is False
        assert inventory_plugin.verify_file("tests/unit/plugins/inventory/inventory.yml") is False
    ...
    def test_execute_query_with_cache(self, inventory_plugin, mock_connection):
        ...
        cached_result = inventory_plugin._execute_query('SELECT * FROM hosts', 'test_key')
    ...
    def test_process_inventory_row_with_host_vars_comma_string(self, inventory_plugin):
        ...
        row = ('app-server-1', ['app_servers'], None, 'ansible_user=deploy,environment=staging')
    ...
    def test_add_host_to_groups_string_comma_separated(self, inventory_plugin):
        ...
        assert 'host1' in inventory_plugin.inventory.groups['app'].hosts
        assert 'host1' in inventory_plugin.inventory.groups['db'].hosts
    ...
    def test_full_parse_integration(self, inventory_plugin, mock_connection):
        ...
        assert db1_vars['replication'] == 'true'
