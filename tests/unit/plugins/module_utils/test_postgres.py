...
class TestEnsureReqLibs:
    ...
    def test_ensure_req_libs_ca_cert_low_psycopg2_ver(
        self, m_ansible_module, m_psycopg2, monkeypatch
    ):
        """
...
class TestConnectToDb:
    ...
    def test_fail_on_conn_true(self, m_ansible_module, monkeypatch, m_psycopg2):
        ...
        conn_params = pg.get_conn_params(m_ansible_module, m_ansible_module.params)
        db_connection, dummy = pg.connect_to_db(
            m_ansible_module, conn_params, fail_on_conn=True
        )
    ...
    def test_fail_on_conn_false(self, m_ansible_module, monkeypatch, m_psycopg2):
        ...
        conn_params = pg.get_conn_params(m_ansible_module, m_ansible_module.params)
        db_connection, dummy = pg.connect_to_db(
            m_ansible_module, conn_params, fail_on_conn=False
        )
    ...
    def test_autocommit_true(self, m_ansible_module, monkeypatch, m_psycopg2):
        ...
        conn_params = pg.get_conn_params(m_ansible_module, m_ansible_module.params)
        db_connection, dummy = pg.connect_to_db(
            m_ansible_module, conn_params, autocommit=True
        )
        cursor = db_connection.cursor()
...
class TestGetConnParams:
    ...
    def test_get_conn_params_def(self, m_ansible_module, m_psycopg2, monkeypatch):
        ...
        assert (
            m_ansible_module.warn_msg
            == "Database name has not been passed, used default database to connect to."
        )

    def test_get_conn_params_warn_db_def_false(
        self, m_ansible_module, m_psycopg2, monkeypatch
    ):
        """Test get_conn_params(), warn_db_default kwarg is False."""
        monkeypatch.setattr(pg, "psycopg", m_psycopg2)
        assert (
            pg.get_conn_params(m_ansible_module, INPUT_DICT, warn_db_default=False)
            == EXPECTED_DICT
        )
